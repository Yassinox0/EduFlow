...
export default function PersonnelFormPage() {
  ...
  return <form className="admin-grid personnel-form" onSubmit={submit}>
    ...
    {form.personnel_type === "TEACHER" && <section className="panel"><h3>Informations professeur</h3><div className="personnel-grid"><label>Spécialité<input value={form.specialty} onChange={(event) => set("specialty", event.target.value)} /></label><fieldset><legend>Matières</legend><div className="check-list">{subjects.map((subject) => <label key={subject.id}><input type="checkbox" checked={form.subject_ids.includes(String(subject.id))} onChange={() => toggle("subject_ids", subject.id)} />{subject.name}</label>)}</div></fieldset><label className="wide">Niveaux ou classes<select multiple value={form.class_level_ids} onChange={(event) => set("class_level_ids", Array.from(event.target.selectedOptions, (option) => option.value))}>{levels.map((level) => <option key={level.id} value={level.id}>{level.name}</option>)}</select><small className="muted">Maintenez Ctrl (Windows) ou ⌘ (Mac) pour sélectionner plusieurs niveaux ou classes.</small></label></div></section>}
