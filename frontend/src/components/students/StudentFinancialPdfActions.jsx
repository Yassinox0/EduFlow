...
import { downloadFamilyPaymentsPdf, downloadFinancialStatementPdf, downloadStudentPaymentsPdf } from "../../services/paymentDocumentService";
...
export default function StudentFinancialPdfActions({ student }) {
  ...
  const download = async (type) => {
    ...
    try { const params = Object.fromEntries(Object.entries(period).filter(([, value]) => value)); if (type === "statement") await downloadFinancialStatementPdf(student.id, params); else if (type === "student") await downloadStudentPaymentsPdf(student.id, params); else await downloadFamilyPaymentsPdf(student.family_id, params); }
