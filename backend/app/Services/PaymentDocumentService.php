...
class PaymentDocumentService
{
    ...
    public function monthly(array $filters): array
    {
        ...
        $stmt = Database::connect()->prepare('SELECT p.id,p.amount_paid,p.payment_date,p.payment_method,COALESCE(pm.label,p.payment_method) payment_method_label,s.first_name,s.last_name,COALESCE(cl.level_name,cl.name,s.class_level) class_level_name,COALESCE(cl.group_name,s.class_name) class_name,COALESCE(f.family_name,s.parent_name,"") responsible_name,mf.total_amount FROM payments p INNER JOIN students s ON s.id=p.student_id AND s.school_id=p.school_id INNER JOIN monthly_fees mf ON mf.id=p.monthly_fee_id AND mf.school_id=p.school_id LEFT JOIN class_levels cl ON cl.id=s.class_level_id AND cl.school_id=s.school_id LEFT JOIN families f ON f.id=s.family_id AND f.school_id=s.school_id LEFT JOIN payment_methods pm ON pm.id=p.payment_method_id WHERE p.school_id=? AND mf.month_label=? AND mf.year_value=? ORDER BY class_level_name,class_name,s.last_name,s.first_name,p.payment_date,p.id');
    ...
    public function unpaid(array $filters): array
    {
        ...
        $stmt = Database::connect()->prepare('SELECT mf.total_amount,mf.amount_paid,mf.remaining_amount,mf.status,s.first_name,s.last_name,COALESCE(cl.level_name,cl.name,s.class_level) class_level_name,COALESCE(cl.group_name,s.class_name) class_name,COALESCE(f.family_name,s.parent_name,"") responsible_name,COALESCE(g.phone_primary,s.phone,"") phone FROM monthly_fees mf INNER JOIN students s ON s.id=mf.student_id AND s.school_id=mf.school_id LEFT JOIN class_levels cl ON cl.id=s.class_level_id AND cl.school_id=s.school_id LEFT JOIN families f ON f.id=s.family_id AND f.school_id=s.school_id LEFT JOIN guardians g ON g.id=f.primary_guardian_id AND g.school_id=f.school_id WHERE mf.school_id=? AND mf.month_label=? AND mf.year_value=? AND mf.status != "PAID" ORDER BY class_level_name,class_name,s.last_name,s.first_name');
    ...
    private function studentRow(int $studentId, int $schoolId): array|false { $stmt=Database::connect()->prepare('SELECT s.*,COALESCE(cl.level_name,cl.name,s.class_level) class_level_name,COALESCE(f.family_name,s.parent_name,"") responsible_name FROM students s LEFT JOIN class_levels cl ON cl.id=s.class_level_id AND cl.school_id=s.school_id LEFT JOIN families f ON f.id=s.family_id AND f.school_id=s.school_id WHERE s.id=? AND s.school_id=?'); $stmt->execute([$studentId,$schoolId]); return $stmt->fetch(); }
    ...
    private function pdf(string $title,string $meta,string $body,string $filename,int $schoolId):array{$html='<!doctype html><html lang="fr"><head><meta charset="UTF-8"><style>@page{margin:18mm 12mm}body{font-family:DejaVu Sans;font-size:9px;color:#1d2b40}h1{font-size:16px;margin:0 0 6px;color:#0f4aa3}h3{font-size:11px;margin:16px 0 7px}.meta,p{line-height:1.45;margin:0 0 10px}table{width:100%;border-collapse:collapse;margin:8px 0 12px}thead{display:table-header-group}th{background:#edf5ff;color:#1f4d86}th,td{border:1px solid #d6dfeb;padding:5px;text-align:left;vertical-align:top}.total{font-weight:bold;text-align:right;padding:8px;background:#f3f8ff}.student-section{page-break-inside:avoid}</style></head><body>'.(new SchoolDocumentHeaderService())->render($schoolId).'<h1>'.$this->e($title).'</h1><div class="meta">'.$meta.'</div>'.$body.'</body></html>'; $options=new Options();$options->set('defaultFont','DejaVu Sans');$options->set('isRemoteEnabled',false);$pdf=new Dompdf($options);$pdf->loadHtml($html);$pdf->setPaper('A4','landscape');$pdf->render();return ['content'=>$pdf->output(),'filename'=>$filename];}
